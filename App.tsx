...
import { generateTryOnImages, enhanceBackgroundPrompt } from './services/geminiService';
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedItemUrls.length; i++) {
        ...
        setLoadingMessage(`Generating for item ${i + 1} of ${selectedItemUrls.length}...`);
        
        const itemPhotoBase64 = itemUrl;

        const image = await generateTryOnImages(bodyPhotoBase64, itemPhotoBase64, customBackgroundPrompt);
        results.push({ itemUrl: itemPhotoBase64, image });
      }
